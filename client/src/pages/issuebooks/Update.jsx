...
const Update = ({ match }) => {
  ...
  useEffect(() => {
    ...
    Promise.all([
      axios.get("https://library-management-6svy.onrender.com/api/books"),
      axios.get("https://library-management-6svy.onrender.com/api/users"),
      axios.get(
        `https://library-management-6svy.onrender.com/api/issuebooks/${match.params._id}`
      ),
    ])
      .then(([booksRes, usersRes, issueRes]) => {
        ...
        setIssueBook({
          ...
          issueDate: issueData.issueDate
            ? issueData.issueDate.slice(0, 10)
            : "",
          returnDate: issueData.returnDate
            ? issueData.returnDate.slice(0, 10)
            : "",
          status: issueData.status,
          fineAmount: issueData.fineAmount,
          actualReturnDate: issueData.actualReturnDate
            ? issueData.actualReturnDate.slice(0, 10)
            : "",
        });
  ...
  const handleReturnBook = () => {
    axios
      .put(
        `https://library-management-6svy.onrender.com/api/issuebooks/${match.params._id}/return`
      )
      .then(() => {
        setRedirect(true);
      })
