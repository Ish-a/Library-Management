...
const List = ({ match }) => {
  ...
  const loadIssuedBooks = () => {
    axios
      .get("https://library-management-6svy.onrender.com/api/issuebooks/")
      .then((response) => {
  ...
  const deleteIssue = (_id) => {
    axios
      .delete(
        `https://library-management-6svy.onrender.com/api/issuebooks/${_id}`
      )
      .then(() => {
        loadIssuedBooks();
      });
      ...
      <Table singleLine columns={6} striped>
        ...
        <Table.Body>
          {issuedBooks.map((issue) => {
            const { _id, bookId, userId, issueDate, returnDate, status } =
              issue;
            return (
              <Table.Row key={_id}>
